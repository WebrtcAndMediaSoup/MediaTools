                                 ...
                                 unsigned long buffer_size)
{
    int ret=0;
	NeAACDecStruct* hDecoder = (NeAACDecStruct*)hpDecoder;
	ret=aac_frame_decode(hDecoder, hInfo, buffer, buffer_size, NULL, 0);
	hInfo->elenum=hDecoder->elenum;
	strcpy(hInfo->elename,hDecoder->elename);
	strcpy(hInfo->eleinfo,hDecoder->eleinfo);
