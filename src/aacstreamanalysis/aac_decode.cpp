    ...
    *song_length = length;
    ...
    do
    {
        ...
        if (first_time && !frameInfo.error)
        {
			...
			CString tempstr;
			//声道数
			tempstr.Format(_T("%d"),frameInfo.channels);
			dlg->AppendBInfo(_T("FrameInfo"),_T("Channels"),tempstr,_T("Channels"));
			//采样率
			tempstr.Format(_T("%d"),frameInfo.samplerate);
			dlg->AppendBInfo(_T("FrameInfo"),_T("Sample Rate"),tempstr,_T("Sample Rate"));
			//AAC类型-------
			tempnum=frameInfo.object_type;
			switch (tempnum){
			case MAIN:tempstr.Format(_T("MAIN"));break;
			case LC:tempstr.Format(_T("LC"));break;
			case SSR:tempstr.Format(_T("SSR"));break;
			case LTP:tempstr.Format(_T("LTP"));break;
			case HE_AAC:tempstr.Format(_T("HE_AAC"));break;
			case ER_LC:tempstr.Format(_T("ER_LC"));break;
			case ER_LTP:tempstr.Format(_T("ER_LTP"));break;
			case LD:tempstr.Format(_T("LD"));break;
			case DRM_ER_LC:tempstr.Format(_T("DRM_ER_LC"));break;
			}
			dlg->AppendBInfo(_T("FrameInfo"),_T("Oject Type"),tempstr,_T("Oject Type"));
			//Header Type
			tempnum=frameInfo.header_type;
			switch (tempnum){
			case RAW:tempstr.Format(_T("RAW"));break;
			case ADIF:tempstr.Format(_T("ADIF"));break;
			case ADTS:tempstr.Format(_T("ADTS"));break;
			case LATM:tempstr.Format(_T("LATM"));break;
			}
			dlg->AppendBInfo(_T("FrameInfo"),_T("Header Type"),tempstr,_T("Header Type"));

			//SBR
			tempnum=frameInfo.sbr;
			switch (tempnum){
			case NO_SBR:tempstr.Format(_T("off"));break;
			case SBR_UPSAMPLED:tempstr.Format(_T("on;upsample"));break;
			case SBR_DOWNSAMPLED:tempstr.Format(_T("on;downsampled"));break;
			case NO_SBR_UPSAMPLED:tempstr.Format(_T("off;upsampled"));break;
			}
			dlg->AppendBInfo(_T("FrameInfo"),_T("SBR"),tempstr,_T("HE-AAC(AAC+SBR)"));

			//PS
			tempnum=frameInfo.ps;
			switch (tempnum){
			case 0:tempstr.Format(_T("off"));break;
			case 1:tempstr.Format(_T("on"));break;
			}
			dlg->AppendBInfo(_T("FrameInfo"),_T("PS"),tempstr,_T("HE-AAC v2(AAC+SBR+PS)"));
		...
		if(frameInfo.channels!=0&&frameInfo.samplerate)
			frame_duration=(frameInfo.samples*1000)/(frameInfo.channels*frameInfo.samplerate);
		dlg->AppendLInfo(frameInfo.bytesconsumed,frameInfo.samples,frame_duration,frameInfo.elenum,frameInfo.elename,frameInfo.eleinfo);


        percent = min((int)(b.file_offset*100)/fileread, 100);

		dlg->m_aaacprogress.SetPos(percent);
...
#define PRINT_MP4_METADATA
	...
	CString tempstr;
	//objectTypeIndex
	tempstr.Format(_T("%d"),mp4ASC.objectTypeIndex);
	dlg->AppendBInfo(_T("Container"),_T("objectTypeIndex"),tempstr,_T("object Type Index"));
	//samplingFrequencyIndex
	tempstr.Format(_T("%d"),mp4ASC.samplingFrequencyIndex);
	dlg->AppendBInfo(_T("Container"),_T("samplingFrequencyIndex"),tempstr,_T("sampling Frequency Index"));
	//samplingFrequency
	tempstr.Format(_T("%d"),mp4ASC.samplingFrequency);
	dlg->AppendBInfo(_T("Container"),_T("samplingFrequency"),tempstr,_T("sampling Frequency"));
	//channelsConfiguration
	tempstr.Format(_T("%d"),mp4ASC.channelsConfiguration);
	dlg->AppendBInfo(_T("Container"),_T("channelsConfiguration"),tempstr,_T("channels Configuration"));
	//-----------
	//输出MetaData
	//此处使用枚举的方法
	//-------------
...
#ifdef _UNICODE
				USES_CONVERSION;
				dlg->AppendBInfo(_T("MetaData"),A2W(item),A2W(tag),A2W(tag));
#else
				dlg->AppendBInfo(_T("MetaData"),item,tag,tag);
#endif
...
int aac_decode(LPVOID lparam,int argc, char *argv[])
{
...
/* System dependant types */
    ...
    if (mp4file)
    {
		dlg->AppendBInfo(_T("Container"),_T("Container"),_T("MP4"),_T("MP4 Container Format"));
		...
		dlg->AppendBInfo(_T("Container"),_T("Container"),_T("RAW"),_T("RAW Format"));
